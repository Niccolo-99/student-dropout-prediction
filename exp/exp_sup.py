...
class Exp_All_Task(object):
    ...
    def _build_model(self, ddp=True):
        ...
        # Verifica se siamo in modalità distribuita
        is_distributed = hasattr(self.args, 'local_rank') and self.args.local_rank is not None
        
        # Applica DistributedDataParallel solo se in modalità distribuita E ddp=True
        if ddp and is_distributed and torch.distributed.is_initialized():
    ...
    def test(self, setting, load_pretrain=False, test_data_list=None, test_loader_list=None):
        ...
        avg_classification_f1 = []
        avg_classification_aucpr = []
        ...
        for task_id, (test_data, test_loader) in enumerate(zip(test_data_list, test_loader_list)):
            ...
            if task_name == 'long_term_forecast':
                ...
                results = self.test_classification(
                    setting, test_data, test_loader, data_task_name, task_id)
                ...
                if is_main_process():
                    ...
                    wandb.log({'eval_CLS-f1_'+data_task_name: f1})
                    wandb.log({'eval_CLS-aucpr_'+data_task_name: aucpr})
                avg_classification_acc.append(acc)
                avg_classification_f1.append(f1)
                avg_classification_aucpr.append(aucpr)

            elif task_name == 'survival':
                c_index = self.test_survival_analysis(
                    setting, test_data, test_loader, data_task_name, task_id)
                total_dict[data_task_name] = {'c_index': c_index}
                if is_main_process():
                    wandb.log({'eval_Survival-cindex_'+data_task_name: c_index})
                avg_survival_cindex.append(c_index)
        ...
        avg_classification_f1 = np.average(avg_classification_f1)
        avg_classification_aucpr = np.average(avg_classification_aucpr)
        ...
        avg_survival_cindex = np.average(avg_survival_cindex) if avg_survival_cindex else 0.0

        if is_main_process():
            wandb.log({
                'avg_eval_LF-mse': avg_long_term_forecast_mse, 
                'avg_eval_LF-mae': avg_long_term_forecast_mae,
                'avg_eval_CLS-acc': avg_classification_acc,
                'avg_eval_CLS-f1': avg_classification_f1,        
                'avg_eval_CLS-aucpr': avg_classification_aucpr,  
                'avg_eval_IMP-mse': avg_imputation_mse, 
                'avg_eval_IMP-mae': avg_imputation_mae,
                'avg_eval_Anomaly-f_score': avg_anomaly_f_score,
                'avg_eval_Survival-cindex': avg_survival_cindex  # AGGIUNGERE
            })

            print("Avg score: LF-mse: {}, LF-mae: {}, CLS-acc {}, CLS-f1 {}, CLS-aucpr {}, IMP-mse: {}, IMP-mae: {}, Ano-F: {}, Surv-C: {}".format(
                avg_long_term_forecast_mse, avg_long_term_forecast_mae, avg_classification_acc, 
                avg_classification_f1, avg_classification_aucpr, avg_imputation_mse, avg_imputation_mae, avg_anomaly_f_score, avg_survival_cindex), folder=self.path)
    ...
    def test_classification(self, setting, test_data, test_loader, data_task_name, task_id):
        ...
        preds = []
        trues = []
        probs = []  # Aggiunta lista per probabilità
        ...
        with torch.no_grad():
            for i, (batch_x, label, padding_mask) in enumerate(test_loader):
                ...
                outputs = torch.nn.functional.softmax(outputs, dim=1)  # AGGIUNTO dim=1

                predictions = torch.argmax(outputs, dim=1)
                preds.append(predictions.detach())
                trues.append(label)
                probs.append(outputs.detach())  # AGGIUNTO: salva le probabilità

        preds = gather_tensors_from_all_gpus(preds, self.device_id, to_numpy=False)
        trues = gather_tensors_from_all_gpus(trues, self.device_id, to_numpy=False)
        probs = gather_tensors_from_all_gpus(probs, self.device_id, to_numpy=False)
        ...
        # F1 score (macro average per multi-class)
        f1 = f1_score(trues_np, predictions, average='macro')
        
        # AUCPR - gestione sia binario che multi-class
        try:
            if probs_np.shape[1] == 2:  # Classificazione binaria
                aucpr = average_precision_score(trues_np, probs_np[:, 1])
            else:  # Multi-class - usiamo macro average
                from sklearn.preprocessing import label_binarize
                # Binarizza le etichette per multi-class AUCPR
                n_classes = probs_np.shape[1]
                trues_bin = label_binarize(trues_np, classes=range(n_classes))
                aucpr = average_precision_score(trues_bin, probs_np, average='macro')
        except Exception as e:
            print(f"Warning: Could not compute AUCPR for {data_task_name}: {e}")
            aucpr = 0.0
        
        torch.cuda.empty_cache()

        print('data_task_name: {} accuracy:{:.4f} f1:{:.4f} aucpr:{:.4f}'.format(
            data_task_name, accuracy, f1, aucpr), folder=self.path)
