...
def init_distributed_mode(args):
    # Check if we're in debug mode (non-distributed)
    is_debug = not hasattr(args, 'local_rank') or getattr(args, 'local_rank', None) is None
    
    if is_debug:
        print("Running in non-distributed mode (debug)")
        return
